...
int main(int argc, char* argv[]) {
	...
	byte* c = (byte*)malloc(BYTES_CNT);
	byte* d = (byte*)malloc(BYTES_CNT);
	byte* e = (byte*)malloc(BYTES_CNT);
	byte* f = (byte*)malloc(BYTES_CNT);
	...
	// byte* tpx = (byte*)malloc(BYTES_CNT);
	// byte* tpy = (byte*)malloc(BYTES_CNT);
	// byte* tqx = (byte*)malloc(BYTES_CNT);
	// byte* tqy = (byte*)malloc(BYTES_CNT);
	// byte* trx = (byte*)malloc(BYTES_CNT);
	// byte* try = (byte*)malloc(BYTES_CNT);
	...
	// start = clock();
	// for(long long x = 0; x < 100000; x++) {
	// 	ECaddPointMonty(ECrx, ECry, tpx, tpy, tqx, tqy);
	// }
	// end = clock();
	...
	// ECaddPoint(a, b, ECpx, ECpy, ECpx, ECpy);
	// ECaddPoint(c, d, a, b, ECpx, ECpy);
	// printHex(c);
	// printHex(d);
	...
	ECSquareAndMultiply(c, d, ECpx, ECpy, f);
	printHex(c);
	printHex(d);
	// ECaddPoint(a, b, ECpx, ECpy, ECpx, ECpy);
	...
	// zero(ECrx);
	// zero(ECry);
	// ECaddPointMonty(ECrx, ECry, ECpx, ECpy, ECpx, ECpy);
	// printHex(ECrx);
	// printHex(ECry);
